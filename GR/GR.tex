...
First, we'll establish some notation to describe how vectors transform under coordinate change. Let's define a four vector
\( x^{\mu} \) where \( x^{0} \) is the temporal component and \( x^{i} \) (\( i \neq 0 \)) are the spacial coordinates.
In these notes, we'll write the representation of a vector in coordinate transformation as \( x^{\mu'} \) (each source
has their own convention). Then the matrix to transform the vector \( x^{\nu} \) into the new primed coordinates is
