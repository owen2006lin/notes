...
\noindent A tensor is a multilinear map of dual vectors and vectors to \( \mathbb{R} \). For example, \( T(a\omega, cV) = acT(\omega, V) \).
...
\begin{itemize}
    ...
    \item \( T \) transforms \( k \) times contravariantly and \( l \) times covariantly: 
...
\]
We can expect then, that in a uniform gravitational field with same magnitude, we should observe the same redshift. 
