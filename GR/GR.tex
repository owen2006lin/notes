...
\newcommand{\bluelink}[2]{\hyperref[#1]{\textcolor{blue}{#2}}}
...
\\
\\
\noindent Some direct consequences of the equivalence principles:
\begin{itemize}
    \item Laws of physics should be expressed in coordinate invariant form. Under coordinate transformation, the form of the equation should remain the same. For example, Maxwell's equations \(\partial_\mu F^{\nu\mu} = J^{\nu}\) is not in covariant form because the
    partial derivative is not a tensor. The coordinate invariant form of this equation is \(\nabla_{\mu}F^{\nu\mu} = J^{\nu}\), which is the unique tensorial equation that is equivalent in inertial coordinates. It is not the unique generalization in curved spacetime
    but additional terms in \(R^\rho{}_{\sigma\mu\nu}\) are  undesirable by \bluelink{Minimal Coupling}{minimal coupling}.

    \item There exist a \bluelink{metric}{metric} on spacetime whose curvature alone is responsible for gravity.
    
    \item There do not exist any other fields that resemble gravity.

    \item The interactions of matter fields to curvature are minimal, and do not involve direct couplings to the \bluelink{Riemann}{Riemann tensor} or its contractions. This is more of an approximation, for example if we suppose the geodesic equation is equal to a factor of the \bluelink{Ricci}{Ricci scalar},
    by dimensional analysis, the correction term is on order of \(10^{-102}\).
\end{itemize}
...
\begin{itemize}
    ...
    \[
        \frac{d}{d\lambda}V^\mu +\Gamma^\mu_{\sigma \rho}\frac{d x^\sigma}{d\lambda}V^\rho = 0
    \]

    \item We define a geodesic as the curve along which a tangent vector is parallel transported. This gives us \(\frac{D}{d\lambda}\frac{dx^\mu}{d\lambda} = 0\) and expanding this gives us
    \[
        \frac{d^2 x^\mu}{d\lambda^2} + \Gamma^\mu_{\rho \sigma}\frac{dx^\rho}{d\lambda}\frac{dx^\sigma}{d\lambda} = 0
    \]
    The so-called \textbf{geodesic equation}. We can see that in euclidean space, the equation reduces trivially with the connection coefficients \(\Gamma^\mu_{\rho \sigma}\) vanishing so we have 
    \(\frac{d^2 x^\mu}{d\lambda^2} = 0\) gives you the equation of a straight line. 

    \item This definition of a geodesic is equivalent to the path that maximizes the proper time, which one can show using variational calculus. This makes sense since in spacetime, such paths are "locally straight".
    
    \item For timelike paths with four velocity \(U^\mu = \frac{dx^\mu}{d\tau}\) the geodesic equation can equivalently be written as 
    \[
        U^\lambda \nabla_\lambda U^\mu = 0
    \]
...
\subsection{Curvature}\label{Riemann}
We already have some notions of curvature by the differences between Euclidean and more general spaces. For example, whether or not parallel transport leaves vectors unchanged on a closed loop. The \textbf{Riemann Tensor} will 
formalize these ideas into a single object. 
\begin{itemize}
    \item Consider an infinitesimal loop defined by two vectors \(A^\mu\) and \(B^\nu\). Imagine parallel transporting some vector \(V^\mu\) along \(A^\mu\), then along \(B^\nu\), then back along \(A^\mu\) and back along \(B^\nu\).
    In curved space the vector will return to its original position and have been changed, and likewise parallel transporting it along the opposite path will also result in changing the vector. The Riemann tensor tells us the difference
    between parallel transporting the vector along the first path and the second path. 

    \item Note that parallel transport is independent of coordinates, so it must be a linear transformation of a vector involving one upper and one lower index. It must also depend on the vectors we used to define the loop, so there must 
    be two additional lower indices to contract with them.

    \item Finally the tensor must be antisymmetric in the two indices.

    \item As per our definition above, the Riemann tensor should be related to commutator of the covariant derivative 
    \[
        [\nabla_\mu , \nabla_\nu]V^\rho = R^\rho{}_{\sigma \mu \nu} - T^\lambda{\mu \nu} \nabla_\lambda V^\rho
    \]
    where \(R^{\rho}{}_{\sigma \mu \nu}\) is the Riemann tensor and \(T^\lambda_{\mu \nu}\) is the Torsion tensor.

    \item Expanding, and eliminating terms that cancel when using the antisymmetric property, we have
    \[
        R^\rho{}_{\sigma \mu \nu} = \partial_\mu \Gamma^\rho_{\nu \sigma} - \partial_\nu \Gamma^\rho_{\mu\sigma} + \Gamma^\rho_{\mu \lambda}\Gamma^\lambda_{\nu \sigma} - \Gamma^\rho_{\nu \lambda}\Gamma^\lambda_{\mu \sigma}
    \]
    This holds for any connection, whether or not it is metric compatible or torsion free.

    \item Using our intuition above, we can think of the Riemann tensor as a map from 3 vector fields to another vector field. The first two arguments should be the fields specifying the "direction" along our infinitessimal loop,
    and the result should act on the third field (the one we're parallel transporting). Using the definition from the commutator, we have  
    
    \[ 
        \boxed{R(X,Y)Z = \nabla_{X}\nabla_{y}Z - \nabla_{Y}\nabla_{X}Z - \nabla_{[X,Y]}Z}
    \]
    
    The notation \( \nabla_{X} \) is the covariant derivative along a vector field \( X \) so we can write it in component form as \( \nabla_{X} = X^{\mu}\nabla_{\mu} \). The last term with the commutator vanishes when \( X \) and \( Y \)
    are coordinate basis vector fields becanse \( [\partial_{\mu}, \partial_{\nu}] = 0 \).
    
\end{itemize}

\noindent Now we will analyze a few properties of the Riemann Tensor in local inertial coordinates; ie the Christoffel symbols drop. It's easier to work with it in all lower indices \( R_{\rho \sigma \mu \nu} = g_{\rho\lambda}R^{\lambda}{}_{\sigma\mu\nu} \):
\begin{itemize}
    \item The Riemann tensor is antisymmetric in the first two indces \( R_{\rho\sigma\mu\nu} = -R_{\sigma\rho\mu\nu}  \) and its last two indices \( R_{\rho\sigma\mu\nu} = -R_{\rho\sigma\nu\mu}  \)
    It is also invariant under changing the first and second pairs of indices: \( R_{\rho \sigma \mu\nu} = R_{\mu \nu \rho \sigma} \).
    
    \item The sum of cyclic permutations of the last three indices vanishes: \( R_{\rho\sigma\mu\nu} + R_{\rho \mu\nu\sigma} + R_{\rho \nu \sigma\mu} = 0 \).
    This is equivalent to saying the antisymmetric part of the last 3 indices is 0: \( R_{\rho[\sigma\mu\nu]} = 0 \) and equivalently \( R_{[\rho\sigma\mu\nu] = 0} \).
    
    \item We can imagine the tensor as a symmetric matrix \( R_{[\rho\sigma][\mu\nu]} \), where each pair corresponds to an antisymmetric matrix. Then we have
    \( \frac{1}{2}[\frac{1}{2}n(n-1)][\frac{1}{2}n(n-1)+1] \) independent components where we used the number of components of an \( m \times m \) symmetric matrix and \( n \times n \) antisymmetric matrix. Finally, the totally antisymmetric component vanishes
    so this reduces the number of independent components to \( \frac{1}{12}n^{2}(n^{2}-1)  \)
    
    \item Taking the covariant derivative, we see the sum of cyclic permutations also vanish so \( \nabla_{\lambda}R_{\rho\sigma\mu\nu} + \nabla_{\rho}R_{\sigma\lambda\mu\nu} + \nabla_{\sigma}R_{\lambda\rho\mu\nu} = 0\). By antisymmetry in the first two indices, we can 
    write 
    \[ 
        \nabla_{[\lambda}R_{\rho\sigma]\mu\nu} = 0
    \]
    known as the \textbf{Bianchi Identity}.


    \item We define the \label{Ricci}\textbf{Ricci Tensor} as the contraction
    ...
    \item The contraction of the Ricci Tensor is the \textbf{Ricci Scalar}:
    \[ 
        R = R^{\mu}{}_{\mu} = g^{\mu\nu}R_{\mu\nu} 
    \]
\end{itemize}

\noindent Finally we define the \textbf{Einstein Tensor} as \( G_{\mu\nu}  = R_{\mu\nu} - \frac{1}{2}Rg_{\mu\nu} \). 

\subsection{Killing Vectors}
Symmetries allow us to solve problems that would otherwise have been painfully difficult. Even if a situation is slightly deviated from a symmetry, we can still account for this through perturbations. We say a manifold \( M \) possesses a symmetry if the metric is perserved under a transformation
mapping \( M \) onto itself. A symmetry of the metric is called an \textbf{isometry}.

\begin{itemize}
    \item Lorentz transformations and translations are isometries of Minkowski space. This is because the metric coefficients \( \eta_{\mu\nu} \) are independent of coordinates \( x^{\mu} \). For some coordinate \( \sigma \):
    
    \[ 
        \partial_{\sigma}g_{\mu\nu} = 0 \implies x^{\sigma} \rightarrow x^{\sigma} + a^{\sigma} \text{is a symmetry} 
    \]    

    \item For a timelike path, the geodesic equation can be written as \( p^{\lambda}\nabla_{\lambda}p^{\mu} = 0 \). By metric compatibility, we can lower the index and expand the covariant derivative to get
    
    \[ 
        p^{\lambda}\partial_{\lambda}p_{\mu} - \Gamma^{\sigma}_{\lambda\mu}p^{\lambda}p_{\sigma} = 0 
    \]
    The first term determines how the components of momentum change along the path \( p^{\lambda}\partial_{\lambda}p_{\mu} = m\frac{dp_{\mu}}{d\tau} \) while the second term reduces to
    \( \frac{1}{2}(\partial_{\mu} g_{\nu\lambda})p^{\lambda}p^{\nu} \) so the geodesic equation can be written as 
    
    \[ 
         m\frac{dp_{\mu}}{d\tau} = \frac{1}{2}(\partial_{\mu} g_{\nu\lambda})p^{\lambda}p^{\nu}
    \]
    But if the metric is independent of the coordinates \( x^{\sigma} \), then the isometry implies \( \frac{dp_{\sigma}}{d\tau} = 0 \) so the momentum four vector is conserved along a geodesic. 

\end{itemize}

\noindent It is important to note that while invariance of the metric implies a symmetry, the converse is not. There are four types of translations and six types of Lorentz transformations, which is ten total symmetries. But this is greater than the number
of dimensions the metric can be independent of. Instead, we'll need a more formal procedure:
\begin{itemize}
    \item Suppose \( g_{\mu\nu} \) is independent of \( x^{\sigma} \). Then let us define \( K = \partial_{\sigma} \), which in component notation is
    
    \[ 
        K^{\mu} = (\partial_{\sigma})^{\mu} = \delta^{\mu}_\sigma 
    \]

    Then \( K^{\mu} \) generates the isometry, namely any transformation of this form can be expressed as motion in the direction of \( K^{\mu} \).

    \item Any four momentum along the direction of \( K^{\nu} \) then obeys \( p_{\sigma} = K^{\nu}p_{\mu} = K_{\nu}p^{\nu} \). Careful! We've specified \( \sigma \) as a specific direction in which the metric is invariant of, so \( p_{\sigma} \) is in fact a scalar;
    it's the component of the four momentum in the direction of \( K^{\nu} \)

    \item The scalar being constant along the path is equivalent to saying the directional derivative along the geodesic is 0, so \( p^{\mu}\nabla_{\mu}(K_{\nu}p^{\nu}) = 0 \). This is because of the chain rule, because covariant derivatives of scalars reduces to the partial 
    derivative. By expanding this equation, we get:
    
    \[ 
        \nabla_{(\mu}K_{\nu)} = 0 \implies p^{\mu}\nabla_{\mu}(K_{\nu}p^{\nu}) = 0 
    \]

    where the left side is \textbf{Killing's equation} and vector fields \( K \) that satisfy it are called \textbf{Killing vector fields}.

    \item Killing vector fields on a manifold are in one-to-one correspondence with the continuous symmetries of the metric on that manifold. 
\end{itemize}


\section{Gravitation}
\subsection{Minimal Coupling}\label{Minimal Coupling}
The motivation behind the use of manifolds was because of several of the postulates established before. The Einstein equivalence principle
stated that in small regions of spacetime, the laws of physics reduce that to special relativity, and that local experiments cannot detect the
existence of gravity. This is very similar to our ability of finding locally inertial coordinates on manifolds; ie where the metric reduces to 
the Minkowski metric and first derivatives vanish. The \textbf{Minimal Coupling Principle} allows us to use this fact to generalize laws of physics
in curved spacetime.

\begin{enumerate}
    \item Take a law of physics in inertial coordinates in flat spacetime
    \item Convert it into a coordinate-invariant tensor equation
    \item Assert the law holds in curved spacetime
\end{enumerate}

\noindent \textbf{Example.} Consider the equation of motion of a free particle travelling in flat space along a parametrized straight line \(x^\mu(\lambda)\). 
This is expressed with the equation 
\[
    \frac{d^2 x^\mu}{d \lambda^2} = 0
\]
\noindent We can rewrite this using the chain rule: \(\frac{d^2 x^\nu}{d \lambda^2} = \frac{d x^\mu}{d \lambda}\partial_\nu \frac{d x^\mu}{d\lambda}\), but the 
partial derivative is not a tensor, so we replace it with the coordinate invariant form; the covariant derivative:
\[
    \frac{d x^\nu}{d\lambda} \nabla_\nu \frac{d x^\mu}{d \lambda} = \frac{d^2x^\mu}{d\lambda^2} + \Gamma^\mu_{\rho \sigma}\frac{dx^\rho}{d\lambda}\frac{dx^\sigma}{d\lambda}
\]

\noindent which is just the geodesic equation. 
\\
\\
\noindent \textbf{Example.}We should also check to see if curvature alone is sufficient to explain gravity in the Newtonian limit. We assume particles are moving slowly, and that
the gravitational field is weak and static. 
\\
\\
...
\[
    \frac{d^2 x^\mu}{d\tau^2} + \Gamma^\mu_{0 0}\left(\frac{dt}{d\tau}\right)^2 = 0
\]
\noindent In a static field, the components of \(g_{\mu\nu}\) are unchanging with time so \(\partial_0 g_{\mu\nu} = 0\), which means the christoffel symbols simplify to
\[
    \Gamma^\mu_{0 0} = -\frac{1}{2}g^{\mu\nu}\partial_\lambda g_{0 0}
\]
Finally, in a weak field, we can represent the metric as a small perturbation in the minkowski metric:
\[
    g_{\mu\nu} = \eta_{\mu\nu} + h_{\mu\nu}
\]
where \(|h_{\mu\nu}| \ll 1\). We can then write the christoffel symbols to first order in \(h_{\mu\nu}\), giving us \(\frac{d^2 x^\mu}{d\tau^2} = \frac{1}{2}\eta^{\mu\nu}\partial_{\lambda}h_{0 0}\left(\frac{dt}{d\tau}\right)^2\)
But since \(\partial_0 g_{0 0} = 0\), the timelike component of the geodesic equation is just \(\frac{d^2 t}{d\tau}^2 = 0\). Meanwhile, the spacial component gives us 
\[
    \frac{d^2 x^i}{dt^2} = \frac{1}{2}\partial_i h_{0 0}
\]
This reduces to the Newtonian limit if we let \(h_{0 0} = -2\mathbf{\Phi}\). You can see this matches Poisson's equation \(\nabla^2 \mathbf{\Phi} = 4\pi G \rho\) exactly. 


\subsection{The Stress-Energy Tensor}

\noindent We'll now introduce the \textbf{stress-energy tensor} $\mathbf{T}^{\mu\nu}$, which is the tensorial equialent to mass.  

\begin{itemize}


    \item We can think of the components of \(T^{\mu\nu}\) as the flux of four momentum \(p^\mu\) across a surface of constant \(x^\nu\). The
    temporal component is \(T^{0 0}\), the flux of \(p^0\) energy across the \(x^0\) time dimension. This is just the rest frame energy density.

    \item \(T^{i 0}\) are the momentum flux across the temporal dimension, which is just the rest momentum density. Meanwhile, \(T^{0 i}\) is the 
    energy flux across a spacial dimension. To show these are equivalent, recall that energy and momentum in relativity are \(E = \gamma m c^2\) and
    \(p = \gamma mv\). Then the energy density \(\epsilon\) and momentum density \(g\) are related by \(g = \frac{1}{c^2}\epsilon v\). If we consider energy
    \(E\) crossing a cuboid of thickenss \(L\) across area \(A\) in time \(\Delta t\), then the energy flux is \(\frac{E}{A\Delta t} = \frac{E}{AL/v} = \epsilon v\),
    which is just the momentum density up to a constant factor. Thus, in the rest frame, the \(T^{i 0}\) and \(T^{0 i}\) components are equivalent.

    \item The \(T^{i j}\) components reduce to the classical stress tensor \(t^{i j}\)from newtonian mechanics. Non diagonal terms represents shear: the ith component of force
    per unit area (pressure) exerted across a surface normal to the jth direction. For a fluid, this could represent forces such as viscocity. The diagonal terms
    give us the pressure \(p_i = T^{i i}\).

    \item For a perfect fluid at rest, the force exerted across a surface is always along the normal and the same in all directions, equivalent
    to pressure $\mathbf{t}^{ij} = p\delta^{ij}$.

\end{itemize}
\noindent \textbf{Example.} Dust is a collection of particles at rest with respect toeach other, travelling at constant four velocity \( U^{{\mu}} \),
We can choose  to move to inertial frame where $U^{\mu} = (1,0,0,0)$. This is called the \textbf{comoving frame}.
\begin{itemize}
    
    \item The \textbf{number current} is $N^{\mu} = nU^{\mu} = n(u^0, \mathbf{u}^i)$ where $n$ is the 
    ...
    is internal energy, then we have $\mu = \mu_0(1+\delta)$ where $\delta$ is the internal energy fraction. The \textbf{momentum density} 
    is $\mathbf{\rho}^i = m_0n\mathbf{u}^{i}$. 
    
    \item In the rest frame, \( \rho \) is the \( \mu = 0 \), \( \nu = 0 \) component of the tensor product \( p^{\mu} \otimes N^{\nu} \) so we conjecture
    that the stress energy tensor for dust is 
...
\noindent \textbf{Example.} A perfect fluid has isotropic rest frame pressure \( p \) and energy density \( \rho \). Because of isotropy, the stress energy tensor
is diagonal
\[ 
    T^{{\mu\nu}} = 
    \begin{pmatrix}
...
\begin{itemize}
    \item For dust, we saw that \( p = 0 \)
    \item An isotropic gas has \( p = \frac{1}{3}\rho \)
    \item The vacuum energy has \( p = -\rho \) which means the stress tensor is proportional to the metric \( T^{\mu\nu} = -\rho_{\text{vac}}\eta^{\mu\nu} \)
    Energy in a vacuum is irrelevent in special relativity because energy can be always be changed up to a scalar value. However, this fact is not true in general relativity
    because all energy couples to gravity.
\end{itemize}

\noindent A covariant expression that reduces to this in the rest frame is then 
...
\noindent Some additional properties of the stress tensor:
\begin{itemize}
    \item As we've seen, \( T^{\mu\nu}\) is symmetric. Additionally, the stress tensor is conserved, meaning it is ``divergence free.''
    \[ 
        \partial_{\mu}T^{\mu\nu} = 0 
    \]

    \item Expanding this equation and contracting in the \( U_{\nu} \) component, we obtain \( U_{\nu}\partial_{\mu}T^{\mu\nu} = -\partial_{\mu}(\rho U^{\mu}) - \partial_{\mu}U^{\mu} = 0 \).
    In the nonrelativistic case,the four velocity is \( U^{\mu}  = (1, v^{i})\), where \( |v^{i}| \ll 1 \) and \( p \ll \rho \). The second term drops and we are left with
    ...
    \item Meanwhile, using the projection operator \( P^{\sigma}{}_{\nu} = \delta^{\sigma}_\nu + U^{\sigma}U_{\nu} \) and applying it to conservation of the stress tensor, eliminating all terms first
    order in \( \mathbf{v} \) which are small, we have
...
\textbf{Derivation of the Einstein Equations}
\\
\noindent Poisson's equation \(\nabla^2 \Phi = 4\pi G\rho\) relates gravitational fields to the mass density. However, we've been building up to the idea that gravity can be explained by curvature alone. Thus, a more
fundamental equation relating curvature and mass.
\\
\\
First, we should replace gravitational potential with a tensorial object; the metric immediately comes to mind, as in the newtonian limit we were able to reproduce gravity
by perturbations in the metric. Meanwhile, the left side is second order in the derivatives of the potential, so we should look for something similarly second order in the metric
that describes curvature; the Einstein tensor.
\\
\\
...
\[
    T_{\mu\nu} = \rho U_\mu U_\nu
\]
where \(U^{mu} = (U^0,0,0,0)\). But in the rest frame, we can take \(U_0 \approx 1\), so we have \(T_{0 0} = \rho\). Note that in the rest frame, the \(T_{0 0}\)
term dominates all the other terms, so the trace to lowest order is just \(    T = g^{0 0}T_{0 0} = -T_{0 0} = -\rho\) which gives us \(R_{0 0} = \frac{1}{2}\kappa \rho\). To solve for the metric, we need to evaluate \(R_{0 0} = R^{\lambda}{}_{0\lambda 0}\) but \(R^0{}_{0 0 0 } = 0\)
so we only need the spacial components. Now we have
\[
    R^{i}_{0 j 0} = \partial_j \Gamma^i_{0 0} - \partial_0 \Gamma^i_{j 0} + \Gamma^i_{j \lambda}\Gamma^\lambda_{0 0}- \Gamma^i_{0 \lambda}\Gamma^\lambda_{j 0}
\]
...
\[
    R_{0 0} = R^i{}_{0 i 0} = \partial_i \left[\frac{1}{2} g^{i \lambda}(\partial_0 g_{\lambda 0} + \partial_0 g_{0 \lambda} - \partial_\lambda g_{0 0})\right]
\]
But to first order and for static fields, the temporal derivatives vanish, reducing the equation to
\[
    = -\frac{1}{2}\delta^{i j}\partial_i \partial_j h_{0 0} = -\frac{1}{2}\nabla^2 h_{0 0} = \frac{1}{2}\kappa \rho
\]
which gives us Poisson's equation if \(\kappa = 8 \pi G\). Thus, we've determined Einstein's equation as 
\[
    \boxed{R_{\mu\nu} - \frac{1}{2}R g_{\mu\nu} = 8\pi G T_{\mu\nu}}
\]
\noindent Using \(R = -8\pi G T\), it's sometimes helpful to write it in this form
\[
    R_{\mu\nu} = 8\pi G\left(T_{\mu\nu} - \frac{1}{2} T g_{\mu\nu}\right)
\]
...
\\
\\
\noindent Now that we have the Einstein field equations, we can examine them more closely:
\begin{itemize}
    \item The metric is a symmetric 4 x 4 tensor which has 10 independent components. The Bianchi identity
    \(\nabla^\mu G_{\mu\nu} = 0\) gives us 4 independent equations so we drop down to 6 independent equations. 
    Another way to think of this is that if a metric is a solution in one coordinate system, it should also be 
    a solution in another coordinate system, so there are four unphysical degrees of freedom in \(g^{\mu\nu}\).

    \item The equations are very complex, since the Ricci scalar and tensor are contractions of the Riemann tensor,
    which itself involves products of the christoffel symbols which themselves involve derivatives of the metric. The
    equations are also nonlinear, meaning the sum of solutions is generally not a solution.

    \item In classical physics, the value of energy doesn't matter since we can always define it with respect to an arbitrary
    reference frame. However, this is not the case with the stress energy tensor. Suppose the vacuum has some rest energy \(T_{\mu\nu}^{(\text{vac})}\).
    This energy should not have a preferred direction so it should locally be Lorentz invariant; ie proportional to the metric
    \[
        T_{\mu\nu}^{(\text{vac})} = -\rho_{\text{vac}}g_{\mu\nu}
    \]
    Using the equation of state \(p_{\text{vac}} = - \rho_{\text{vac}}\), einstein's equation becomes 
    \[
        R_{\mu\nu} = \frac{1}{2}Rg_{\mu\nu} + \Lambda G_{\mu\nu} = 8\pi G T_{\mu\nu}
    \]
    where the cosmological constant \(\Lambda\) is equal to the vacuum energy density \(\rho_{\text{vac}} = \frac{\Lambda}{8\pi G}\)
