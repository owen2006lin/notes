...
Poisson's equation relates gravitational fields to the mass density. However, we've been building up to the idea that gravity can be explained by curvature alone. Thus, a more
fundamental equation relating curvature and mass.
\begin{itemize}
    \item First, we should replace gravitational potential with a tensorial object; the metric immediately comes to mind, as in the newtonian limit we were able to reproduce gravity
    ...
    that describes curvature; the Riemann curvature tensor.

    \item The tensorial equivalent of mass is the stress energy tensor so we may postulate
    \[
        R_{\mu\nu} = \kappa T_{\mu\nu}
    \]
    where we've contracted the Riemann tensor into the Ricci tensor so that the number of indices line up. 
\end{itemize}
