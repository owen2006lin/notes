...
\noindent \textbf{Example.} Suppose \(\psi\) is a solution to the Schr\"odinger equation with energy E. 
\\

\noindent Then \(\hat{H}(\hat{a}_{+}\psi) = \hbar\omega \hat{a}_{+} \left(\hat{a}_{-}\hat{a}_{+} + \frac{1}{2}\right)\psi
= \hat{a}_+ \left[\hbar \omega\left(\hat{a}_+\hat{a}_- + 1 + \frac{1}{2}\right)\psi\right] 
= \hat{a}_+ \left(\hat{H}  +\hbar\omega\right)\psi = (E + \hbar\omega)(\hat{a}_+ \psi)\). Similarly, \(\hat{H}(\hat{a}_-\psi)
= (E - \hbar\omega)(\hat{a}_-\psi)\). So this means \(\hat{a}_+\psi\) is a solution with energy \(E + \hbar\omega\) and \(\hat{a}_-\psi\)
is also a solution with energy \(E - \hbar\omega\)
